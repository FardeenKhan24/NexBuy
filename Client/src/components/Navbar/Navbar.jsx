...
const Navbar = () => {
  ...
  const likedProductDetails = allProducts.filter(product =>
  ...
  return (
    ...
    <nav className="navbar-container">
      ...
      <div className="ham" onClick={handleHam}>
        ...
        <Link to="/"><TbShoppingBagSearch className='bag-icon' /></Link>
        <ul className={`navbar-title ${hamburger ? 'mobile-menu' : ''}`}>
          <Link to="/"><li onClick={() => setHamburger(false)}>Home</li></Link>
          <Link to="/about"><li onClick={() => setHamburger(false)}>About</li></Link>
          <Link to="/products"><li onClick={() => setHamburger(false)}>Products</li></Link>
        </ul>
      </div>
      ...
      <div>
        ...
        <li onClick={() => setShowLikedModal(true)}> 
            <FaHeart className='hearts' style={{ color: likedProducts.length > 0 ? "red" : "" }} />
          </li>          
          <Link to="/cart">
      ...
      {profile && (
        <div className="profile">
          {user ? (
            <>
              <li><Link to="/profile" onClick={() => setProfile(false)}>My Profile</Link></li>
              <li onClick={() => { handleLogout(); setProfile(false); }}><Link to="/login">Log Out</Link></li>
            </>
          ) : (
            <li><Link to="/login" onClick={() => setProfile(false)}>Log In</Link></li>
          )}
        </div>
      )}
    </nav>
